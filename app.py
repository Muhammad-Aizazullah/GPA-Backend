...
# Grade to GPA mapping (based on 4.0 scale)
grade_mapping = {
    "A+": 4.0, "A": 4.0, "A-": 3.7,
    "B+": 3.3, "B": 3.0, "B-": 2.7,
    "C+": 2.3, "C": 2.0, "C-": 1.7,
...
def add_course():
    ...
    try:
        ...
        return jsonify({"error": "Credit hours must be a number"}), 400

    grade_point = grade_mapping.get(grade.upper(), None)
...
# For local development only. Use WSGI (gunicorn) for production.
